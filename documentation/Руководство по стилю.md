...
Это руководство по стилю с рекомендациями по идиомам и узорам для написания актеров Акки. Обратите внимание, что данное руководство не охватывает классический API актеров.

Как и во всех руководствах по стилям, относитесь к нему как к списку правил, которые необходимо нарушить. Конечно, бывают моменты, когда альтернативные стили должны быть предпочтительнее приведенных здесь.

## Функциональный против объектно-ориентированного стиля

Есть два вкуса `Actor API`:

1. Стиль функционального программирования, при котором вы передаете функцию на фабрику, которая затем конструирует поведение, для статичных акторов это означает передачу неизменного состояния в качестве параметров и переход к новому поведению всякий раз, когда вам нужно действовать в измененном состоянии.
...
Пример встречного агента, реализованного в функциональном стиле:
...
* Оба они имеют статический метод `создания` фабрики для создания исходного поведения, т.е. со стороны они используются одинаковым образом.
...
* Объектно-ориентированный стиль должен использовать новый экземпляр начального `Behavior` для каждого порожденного экземпляра агента, так как состояние в экземпляре `AbstractBehavior` не должно делиться между экземплярами агентов. Это "скрыто" в стиле функции, так как непреложное состояние захватывается функцией.

* В объектно-ориентированном стиле можно вернуть `это`, чтобы остаться с тем же самым поведением для следующего сообщения. В функциональном стиле нет `этого`, поэтому вместо него используется `Beviors.same`.

* Доступ к `ActorContext` осуществляется с помощью `Behaviors.setup`, но затем сохраняется по-разному. В качестве поля экземпляра в сравнении с параметром метода.

Какой стиль вы выберете, это вопрос вкуса и оба стиля могут быть смешаны в зависимости от того, какой из них лучше всего подходит для конкретного актера. Актер может переключаться между стилями поведения, реализованными в различных стилях. Например, он может иметь начальное поведение, которое заключается только в том, чтобы спрятать сообщения до тех пор, пока не будет выполнен некоторый начальный запрос, а затем перейти к своему основному активному поведению, которое поддерживает некоторое мутирующее состояние. Такое начальное поведение приятно в функциональном стиле, а активное поведение может быть лучше в объектно-ориентированном стиле.

Мы бы порекомендовали использовать инструмент, который лучше всего подходит для этой задачи. API похожи во многих отношениях, чтобы облегчить изучение и того, и другого. Конечно же, вы можете принять решение придерживаться только одного стиля из соображений последовательности и знакомства.
...
* Состояние является непреложным и может быть передано "следующему" поведению.
  `Behavior` является апатридом.
* Жизненный цикл актера имеет несколько различных фаз, которые могут быть представлены переключением между различными типами поведения, например, конечное состояние машины. Это также поддерживается объектно-ориентированным стилем, но, как правило, он более приятен с функциональным стилем.
...
В качестве первого шага мы можем поместить все эти параметры в класс таким образом, что нам придется, по крайней мере, передать только одну вещь. Все равно хорошо иметь "меняющееся" состояние, `финал int n` здесь, в качестве отдельного параметра.
...
Мы можем сделать лучше, введя класс ограждения, даже если он все еще использует функциональный стиль. Параметры "конструктора" могут быть `final` полями экземпляра и доступны из методов-членов.
...
Очень мило. Здесь следует быть осторожным, так как важно создать новый экземпляр для каждого порожденного агента, так как эти параметры не должны разделяться между разными экземплярами агентов. Это естественно при создании экземпляра из `Behaviors.setup`, как в приведенном выше примере. Рекомендуется иметь статический заводской метод `создания` и сделать конструктор приватным.

Это также может быть полезно при тестировании поведения путем создания тестового подкласса, который переопределяет определенные методы в классе. Тест создаст экземпляр без статического метода `create` factory. Затем необходимо ослабить ограничения видимости конструктора и методов.

Не рекомендуется помещать в огораживающий класс мутирующие члены состояния и нефинализирующие члены. С точки зрения потокобезопасности агентов это было бы корректно, если один и тот же экземпляр класса-оболочки не разделяется между различными экземплярами агентов, но если это то, что вам нужно, то лучше использовать объектно-ориентированный стиль с классом `AbstractBehavior`.

## Behavior фабричный метод
